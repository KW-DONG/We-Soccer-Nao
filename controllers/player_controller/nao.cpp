...
Vec3D Nao::readGPS()
{
	return (Vec3D)pGPS->getValues();;
}

Vec3D Nao::readInertial()
{
	return (Vec3D)pInertialUnit->getRollPitchYaw();
}

// walk towards the target 移动到target的位置
bool Nao::move(Vec2D target)
{
	//motion_stop();
	//stand();
	//int times = 5;
...
bool Nao::kick_towards(Vec2D target, Vec2D ballposition)
{
